...
export default function Home() {
  const [term, setTerm] = useState(ATTACKS[0]);
  const [selected, setSelected] = useState(ATTACKS[0]);
  return (
    <Fragment>
      <div className={"grid grid-cols-12 items-end m-4 gap-4"}>
        <Input setTerm={setTerm} value={selected.value} />
        <Select selected={selected} setSelected={setSelected} />
        <Button onClick={() => setTerm(selected)}>Start</Button>
      </div>
      <ShowResult id={term.id} term={term.value} />
    </Fragment>
  );
}
        ...
        return (
          <article className="relative pt-16 prose lg:prose-xl mx-auto text-center">
            ...
            <p>
              ...
              verify by changing the sleep time. For MySQL SLEEP(<i>sec</i>)
              query can be used.
            </p>
          </article>
        );
